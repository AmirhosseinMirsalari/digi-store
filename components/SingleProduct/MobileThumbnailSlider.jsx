...
export default function MobileThumbnailSlider({
  modalMobileSwiper,
  setModalMobileSwiper,
  product,
}) {
  const [thumbsSwiper, setThumbsSwiper] = useState(null);
  const [activeSlider, setActiveSlider] = useState(0);
  const productImageLength = product.product_images.data.map(c=>c.attributes).length
  const productVideoLength = product.product_videos.data.map(c=>c.attributes).filter(
    (a) => a.video != ""
  ).length;
  const videoRef = useRef();

  const modalImageHandler = (e) => {
