...
const SidebarSubCategoryNav = ({ category, indexs, mainCategorySlug }) => {
  ...
  return (
    <div className="bg-[#f1f2f4] pr-8 border border-x-0 border-t-0 mx-8">
      <li
        className={`flex items-center justify-between font-bold text-[13px] cursor-pointer text-[#3f4064] h-[52px]  ${
          openCategoryNavbar ? "text-[#ef4056]" : "text-black"
        }`}
        key={indexs}
      ...
      <div className={`  ${openCategoryNavbar ? "block" : "hidden"}`}>
        ...
        {category.sub_categories.data
          .map((c) => c.attributes)
          .map((subCategory, inde) => (
            <div key={inde} className="h-[52px] flex items-center">
              <p className="text-[#3f4064] text-xs !font-normal">
                {subCategory.name}
              </p>
            </div>
          ))}
