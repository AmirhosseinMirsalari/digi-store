...
export default function HomePage({
  ...
  const filtersData =
    ...
    filterProducts
      ...
      .filter(
        (filters) =>
          filters.subCategorySlug == query || filters.CategorySlug == query
      );
  ...
  return (
    <div className="flex flex-col w-full h-auto ">
      <Head>
        <title>{`محصولات دسته بندی ${router.query.search}`}</title>
      </Head>
                ...
                <div>
                  {brands?.map((brand) => {
                    return (
                      ...
                      >
                        <input
                          onClick={() => clickBrand(brand.createdAt)}
                  ...
                  <div className="border border-x-0 border-t-0 last:border-b-0">
                    ...
                    <div
                      className={`flex flex-col w-full h-auto items-center border border-x-0 border-t-0 ${
                        openValue == filter.createdAt
                          ? "max-h-[200px] scrollCustomer overflow-y-scroll visible"
                          : "!invisible !h-0"
                      } `}
                    >
                      {/* {filter.product_values.data.map((item) => item.attributes).length > 0 &&
                        <input
      ...
      >
        <div className="flex flex-col">
          ...
          <div
            ...
            onClick={closeModalFilterHandler}
          >
              ...
              {brands?.map((brand) => {
                return (
                  <div
                    key={brand.createdAt}
                    className={`w-full border border-x-0 border-t-0 flex items-center ${
                      openBrand ? "max-h-auto visible" : "!h-0 max-h-0 !invisible"
                    } `}
                  >
                    <input
                      className="w-[15px] h-[15px] ml-4 py-1 accent-[#008eb2] cursor-pointer"
          ...
          {/* تخفیف */}
          <div className="flex py-3 border border-x-0 border-t-0 justify-between font-bold text-[15px] text-[#424750]">
            <label className="grow cursor-pointer" htmlFor="defaults-toggles">
              فقط کالا های تخفیف دار
            </label>
          ...
          <div className="flex py-3 border border-x-0 border-t-0 justify-between font-bold text-[15px] text-[#424750]">
            <label className="grow cursor-pointer" htmlFor="default-toggless">
              فقط کالا های موجود
            </label>
...
export async function getServerSideProps({ params }) {
  ...
  let productss = data
    .map((a) => a)
    .filter((product) =>
      product.subCategorySlug == query
        ? product.subCategorySlug == query
        : product.categorySlug == query
        ? product.categorySlug == query
        : null
    );
